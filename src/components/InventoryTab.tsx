...
export default function InventoryTab({
 ...
 // P3 declutter — category/sort live in the Filter popover instead of the main row.
 ...
 {t.medicineInventoryLedger}
  ...
  <TabIntro tabKey="inventory">{t.secureLedgerDescription}</TabIntro>
  </p>
 </div>
 ...
 />
  ...
  {(['all','low','out','discrepancy'] as const).map(sf => {
  const meta = {
  all: { active: 'bg-white border-brand-500 text-brand-700 shadow-sm', idle: 'bg-[#F4F7F5] border-transparent text-slate-500 hover:text-slate-700 hover:bg-slate-100', label: lang === 'ar' ? 'جميع المواد' : 'All Items' },
  low: { active: 'bg-amber-50 border-amber-300 text-amber-700 shadow-sm', idle: 'bg-[#F4F7F5] border-transparent text-slate-500 hover:text-amber-700 hover:bg-amber-50', label: lang === 'ar' ? 'تنبيه نقص المخزون' : 'Low Stock Alert' },
  out: { active: 'bg-rose-50 border-rose-300 text-rose-700 shadow-sm', idle: 'bg-[#F4F7F5] border-transparent text-slate-500 hover:text-rose-700 hover:bg-rose-50', label: lang === 'ar' ? 'نفذت الكمية' : 'Out of Stock' },
  discrepancy: { active: 'bg-red-100 border-red-400 text-red-800 shadow-sm', idle: 'bg-[#F4F7F5] border-transparent text-slate-500 hover:text-red-800 hover:bg-red-50', label: lang === 'ar' ? 'فروقات الجرد (سالب)' : 'Discrepancies' }
  }[sf];
  return (
  <button
  key={sf}
  onClick={() => setStockFilter(sf)}
  className={`px-4 py-2 rounded-xl text-xs font-bold transition-all border whitespace-nowrap flex items-center gap-2 ${stockFilter === sf ? meta.active : meta.idle}`}
  >
  {sf === 'discrepancy' && <AlertTriangle className="w-3 h-3" />}
  {meta.label}
  </button>
  );
  })}
  {/* P3 declutter: category + sort fold into one popover */}
  <div className="relative ms-auto shrink-0">
  <button
  id="btn-inventory-filters"
  onClick={() => setShowFilters(v => !v)}
  className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold border transition-all cursor-pointer ${categoryFilter !== 'All' ? 'bg-brand-50 border-brand-300 text-brand-700' : 'bg-[#F4F7F5] border-transparent text-slate-500 hover:text-slate-700 hover:bg-slate-100'}`}
  >
  <SlidersHorizontal className="w-3.5 h-3.5" />
  {lang === 'ar' ? 'تصفية' : 'Filter'}
  {categoryFilter !== 'All' && <span className="w-1.5 h-1.5 rounded-full bg-brand-600" />}
  </button>
  {showFilters && (
  <>
  <div className="fixed inset-0 z-30" onClick={() => setShowFilters(false)} />
  <div className="absolute end-0 top-full mt-2 z-40 w-64 bg-white border border-brand-100 rounded-xl shadow-lg p-3 space-y-3">
  <div>
  <span className="text-[10px] font-mono font-bold text-slate-400 uppercase">{t.catLabel}</span>
  <select
  id="category-filter"
  value={categoryFilter}
  onChange={(e) => setCategoryFilter(e.target.value)}
  className="mt-1 w-full bg-[#F4F7F5] border border-brand-100/80 focus:border-brand-500/50 rounded-xl py-2 px-3 text-xs text-slate-700 font-semibold focus:outline-none transition-all appearance-none cursor-pointer"
  >
  {CATEGORIES.map(cat => (
  ...
  <div>
  <span className="text-[10px] font-mono font-bold text-slate-400 uppercase">{t.sortLabel}</span>
  <div className="mt-1 flex gap-2">
  <select
  id="sort-by"
  value={sortBy}
  onChange={(e) => setSortBy(e.target.value as any)}
  className="flex-1 bg-[#F4F7F5] border border-brand-100/80 focus:border-brand-500/50 rounded-xl py-2 px-3 text-xs text-slate-700 font-semibold focus:outline-none transition-all appearance-none cursor-pointer"
  >
  ...
  className="px-3 rounded-xl border border-brand-100/80 bg-[#F4F7F5] hover:bg-slate-100 text-slate-500 hover:text-brand-950 transition-colors cursor-pointer"
  >
